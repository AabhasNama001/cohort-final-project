...
const searchProduct = tool(
  async ({ query, token }) => {
    try {
      ...
      return JSON.stringify({ ok: false, error: err?.response?.data || err?.message })
    }
  },
  {
    name: "searchProduct",
    ...
    schema: z.object({
      ...
      token: z.string().optional().describe('Optional bearer token for authenticated requests'),
    }),
  }
)
...
const addProductToCart = tool(
  async ({ productId, qty = 1, token }) => {
    try {
      ...
      return JSON.stringify({ ok: true, data: response.data, message: `Added product ${productId} (qty:${qty})` })
    } catch (err) {
      console.error('addProductToCart error', err?.response?.data || err?.message)
      const status = err?.response?.status
      // return structured error so agent can decide next steps (e.g., ask user to login)
      return JSON.stringify({ ok: false, status, error: err?.response?.data || err?.message })
    }
  },
  {
